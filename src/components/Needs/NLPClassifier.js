...
function NLPClassifier({ onClassified }) {
  ...
  const handleClassify = () => {
    if (!text.trim()) return;
    setLoading(true);
    // simulate slight delay for AI effect
    setTimeout(() => {
      const r = nlpClassify(text);
      const urgencyScore = detectUrgencyScore(text);
      setResult({ ...r, urgencyScore });
      setLoading(false);
      if (onClassified) onClassified({ ...r, title: text });
    }, 600);
  };
    ...
    <div className={styles.box}>
      <div className={styles.header}>
        ...
        <div>
          ...
          <div className={styles.headerSub}>Describe your need in plain language — AI extracts category & urgency</div>
        </div>
      </div>
      ...
      {result && (
        <div className={styles.result}>
          ...
          <div className={styles.resultBadges}>
            ...
            {result.qty && <Badge text={`Qty ~${result.qty}`} color="#3b82f6" size="md" />}
          </div>
          <div className={styles.urgencyBar}>
            ...
            <div className={styles.bar}>
              <div
                ...
                style={{ width: `${result.urgencyScore}%`, background: URGENCY_COLORS[result.urgency] }}
              />
            </div>
            <span className={styles.urgencyVal}>{result.urgencyScore}/100</span>
          </div>
          <div className={styles.hint}>✅ Form below has been pre-filled. Review and submit.</div>
        </div>
      )}
      ...
      <div className={styles.chips}>
        {[
          ...
          <button key={q} className={styles.chip} onClick={() => { setText(q); setResult(null); }}>
            {q}
          </button>
        ))}
      </div>
